...
class RM_Email_Service
{
    ...
    public static function notify_submission_to_admin($params,$token='')
    {
        ...
        $primary_email= RM_DBManager::get_primary_fields_id($params->form_id,'email');
        $primary_email_value='';
        if(count($primary_email)){
            $primary_email_value= isset($params->sub_data[$primary_email[0]]) ? $params->sub_data[$primary_email[0]]->value : '';
        }
        if(empty($primary_email_value)){
        $from_email = $gopt->get_value_of('senders_email_formatted');
         $rm_email->from($primary_email_value); 
        }
        else{
            $disp_name = $gopt->get_value_of('senders_display_name');
            $from_email= $primary_email_value;
            if ($disp_name && $from_email)
