...
<table class="shop_table shop_table_responsive woocommerce-cart-form__contents woocommerce-checkout-review-order-table table table-striped table-hover">
	<thead>
		<tr>
			
			<th class="product-total"><?php _e( 'Product', 'woocommerce' ); ?></th>
   ...
   <tfoot>
    <tr>
        <td colspan="2">
            <a class="button wc-backward" href="<?php echo wc()->cart->get_cart_url();?>">
			  &nbsp;&nbsp;Edit Order</a>
        </td>
        <td colspan="2">
	<table class="rp-checkout-subtotol" style="width:100%;">
...
function remove_responsive(x) {
   if (x.matches)
{
    jQuery(".shop_table").removeClass("shop_table_responsive");    
   
}else{
    jQuery(".shop_table").addClass("shop_table_responsive");
...
remove_responsive(x) ;
remove_responsive(y);
x.addListener(remove_responsive);
y.addListener(remove_responsive);   
});
</script>
<style>
    ...
    @media only screen and (max-width:768px){
            .rp-checkout-subtotol p{font-size: 12px;}
    }
